...
function AttachmentsPage() {
  ...
  const [purchaseId, setPurchaseId] = useState(requestedPurchaseId);
  ...
  const upload = (file: File) => {
    ...
    reader.onload = () => {
      ...
      setMessage("Salvando anexo…");
       create.mutate({ data: { purchaseId: activePurchase ? Number(activePurchase) : null, quotationName: activePurchase ? null : quotationName.trim(), attachmentName: attachmentName.trim(), fileName: file.name, mimeType: file.type || "application/octet-stream", fileSize: file.size, data } }, {
         onSuccess: () => { setMessage("Anexo salvo."); setAttachmentName(""); setQuotationName(""); queryClient.invalidateQueries({ queryKey: getListAttachmentsQueryKey() }); },
  ...
  return <div className="mx-auto max-w-[1440px] animate-rise">
     <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mono text-[11px] uppercase tracking-[.2em] text-[#818cf8]">Biblioteca</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.06em] text-[#f8fafc] sm:text-[42px]">Anexos<span className="text-[#818cf8]">.</span></h1><p className="mt-2 text-sm text-[#94a3b8]">Notas fiscais, comprovantes e cotações em um só lugar.</p></div><div className="flex flex-col items-stretch gap-2 self-start sm:flex-row sm:items-center sm:flex-wrap sm:justify-end"><label className="relative"><span className="sr-only">Vínculo do anexo</span><select data-testid="select-attachment-link" value={purchaseId || (purchases[0] ? String(purchases[0].id) : "standalone")} onChange={(e) => setPurchaseId(e.target.value)} className="focus-ring h-11 w-full max-w-[240px] appearance-none rounded-xl border border-[#334155] bg-[#1e293b] px-3 pr-8 text-xs font-bold text-[#cbd5e1] outline-none"><option value="standalone">Cotação avulsa · sem compra</option>{purchases.map((purchase) => <option key={purchase.id} value={purchase.id}>{purchase.productName}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute right-2 top-3.5 text-[#64748b]" /></label><input data-testid="input-attachment-name" required value={attachmentName} onChange={(e) => setAttachmentName(e.target.value)} placeholder="Nome do anexo" aria-label="Nome do anexo" className="focus-ring h-11 w-full max-w-[210px] rounded-xl border border-[#334155] bg-[#0f172a] px-3 text-xs text-[#f8fafc] outline-none placeholder:text-[#64748b]" />{!activePurchase && <input data-testid="input-quotation-name" required value={quotationName} onChange={(e) => setQuotationName(e.target.value)} placeholder="Nome da cotação" aria-label="Nome da cotação avulsa" className="focus-ring h-11 w-full max-w-[210px] rounded-xl border border-[#334155] bg-[#0f172a] px-3 text-xs text-[#f8fafc] outline-none placeholder:text-[#64748b]" />}<input ref={inputRef} type="file" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload(file); e.target.value = ""; }} /><button data-testid="button-upload-attachment" onClick={() => inputRef.current?.click()} disabled={create.isPending} className="focus-ring flex h-11 items-center justify-center gap-2 rounded-xl bg-[#6366f1] px-4 text-sm font-bold text-[#f8fafc] shadow-[0_8px_18px_rgba(99,102,241,.2)] transition hover:bg-[#818cf8] disabled:opacity-50"><Upload size={17} /> Upload anexo</button></div></div>
    <section className="mt-9 overflow-hidden rounded-xl border border-[#334155] bg-[#1e293b] shadow-[0_18px_40px_rgba(2,6,23,.2)]">
