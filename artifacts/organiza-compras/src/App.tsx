...
function Workspace({ onLogout }: { onLogout: () => void }) {
  ...
  const goAttachments = (purchaseId?: number) => setLocation(purchaseId ? `/attachments?purchaseId=${purchaseId}` : "/attachments");
  return (
      ...
      <div className="mx-auto flex min-h-[100dvh] max-w-[1600px]">
        ...
        <div className="min-w-0 flex-1">
          ...
          <main className="px-5 py-8 pb-28 sm:px-8 lg:px-12 lg:py-11">
            {tab === "overview" && <Overview onAdd={() => go("add")} onAll={() => go("purchases")} onAttachments={goAttachments} onHistory={() => go("history")} />}
            {tab === "purchases" && <AllPurchases onAdd={() => go("add")} onAttachments={goAttachments} />}
            {tab === "attachments" && <AttachmentsPage />}
            {tab === "add" && <PurchaseEntry onDone={() => go("overview")} onAttachments={goAttachments} />}
...
function Overview({ onAdd, onAll, onAttachments, onHistory }: { onAdd: () => void; onAll: () => void; onAttachments: (purchaseId?: number) => void; onHistory: () => void }) {
  ...
  return <div className="mx-auto max-w-[1240px]">
    ...
    {summary?.overdue ? <div className="mt-4 flex items-center gap-3 rounded-xl border border-[#f59e0b]/30 bg-[#f59e0b]/10 px-5 py-4 text-sm text-[#fcd34d]"><AlertCircle size={18} /><span><strong>{summary.overdue} compra{summary.overdue > 1 ? "s" : ""}</strong> passou do prazo previsto. Vale conferir.</span></div> : null}
     {editing && <EditModal purchase={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); refresh(); }} onAttachments={onAttachments} />}
  </div>;
}
...
function AllPurchases({ onAdd, onAttachments }: { onAdd: () => void; onAttachments: (purchaseId?: number) => void }) {
  const queryClient = useQueryClient();
  const purchasesQuery = useListPurchases();
  const del = useDeletePurchase();
  const update = useUpdatePurchase();
  ...
  return <div className="mx-auto max-w-[1440px]">
    <div className="animate-rise flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mono text-[11px] uppercase tracking-[.2em] text-[#818cf8]">Acervo</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.06em] text-[#f8fafc] sm:text-[42px]">Todas as compras<span className="text-[#818cf8]">.</span></h1><p className="mt-2 text-sm text-[#94a3b8]">Busque, filtre e mantenha seus pedidos organizados.</p></div><button onClick={onAdd} className="focus-ring flex h-11 items-center justify-center gap-2 self-start rounded-xl bg-[#6366f1] px-4 text-sm font-bold text-[#f8fafc] transition hover:bg-[#818cf8]"><Plus size={17} /> Nova compra</button></div>
     ...
     {editing && <EditModal purchase={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); invalidate(); }} onAttachments={onAttachments} />}
  </div>;
}
...
function AttachmentsPage() {
  ...
  const [search, setSearch] = useState("");
  const requestedPurchaseId = new URLSearchParams(location.split("?")[1] ?? "").get("purchaseId") ?? "";
  const [purchaseId] = useState(requestedPurchaseId);
  ...
  const purchases = purchasesQuery.data ?? [];
  const attachments = (attachmentsQuery.data ?? []).filter((item) => `${item.attachmentName ?? ""} ${item.fileName} ${item.purchaseName} ${item.quotationName ?? ""}`.toLowerCase().includes(search.toLowerCase()));
  const activePurchase = purchases.find((purchase) => String(purchase.id) === purchaseId) ?? null;
  const resetModal = () => {
    setModalOpen(false);
  ...
  const upload = (event: FormEvent) => {
    ...
    if (!file) { setMessage("Selecione um arquivo antes de enviar."); return; }
    if (!attachmentName.trim()) { setMessage("Informe um nome para o anexo antes de enviar o arquivo."); return; }
    if (!attachmentDate) { setMessage("Informe a data do anexo antes de enviar o arquivo."); return; }
    ...
    reader.onload = () => {
      ...
      setMessage("Salvando anexo…");
       create.mutate({ data: { purchaseId: activePurchase?.id ?? null, quotationName: activePurchase ? null : quotationName.trim(), attachmentName: attachmentName.trim(), attachmentDate, fileName: file.name, mimeType: file.type || "application/octet-stream", fileSize: file.size, data } }, {
         onSuccess: () => { resetModal(); setMessage("Anexo salvo."); queryClient.invalidateQueries({ queryKey: getListAttachmentsQueryKey() }); },
  ...
  const remove = (item: Attachment) => {
     if (!window.confirm(`Excluir o anexo "${item.attachmentName || item.fileName}"?`)) return;
  ...
  return <div className="mx-auto max-w-[1440px] animate-rise">
     <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mono text-[11px] uppercase tracking-[.2em] text-[#818cf8]">Biblioteca</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-.06em] text-[#f8fafc] sm:text-[42px]">Anexos<span className="text-[#818cf8]">.</span></h1><p className="mt-2 text-sm text-[#94a3b8]">Notas fiscais, comprovantes e cotações em um só lugar.</p></div><button data-testid="button-upload-attachment" onClick={openModal} disabled={create.isPending} className="focus-ring flex h-11 items-center justify-center gap-2 self-start rounded-xl bg-[#6366f1] px-4 text-sm font-bold text-[#f8fafc] shadow-[0_8px_18px_rgba(99,102,241,.2)] transition hover:bg-[#818cf8] disabled:opacity-50"><Upload size={17} /> Enviar anexo</button></div>
    <section className="mt-9 overflow-hidden rounded-xl border border-[#334155] bg-[#1e293b] shadow-[0_18px_40px_rgba(2,6,23,.2)]">
        <div className="flex flex-col gap-4 border-b border-[#334155] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 className="text-lg font-extrabold text-[#f8fafc]">Todos os anexos ({attachments.length})</h2><p className="mt-1 text-xs text-[#94a3b8]">Cada arquivo recebe um nome e uma data para ficar fácil de encontrar depois.</p></div><label className="relative w-full sm:max-w-[280px]"><Search size={16} className="absolute left-3 top-3.5 text-[#64748b]" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar anexos ou cotações" className="focus-ring h-11 w-full rounded-xl border border-[#334155] bg-[#0f172a] pl-10 pr-3 text-sm text-[#f8fafc] outline-none placeholder:text-[#64748b]" /></label></div>
       ...
       {attachmentsQuery.isLoading ? <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="skeleton h-64 rounded-xl" />)}</div> : attachmentsQuery.isError ? <ErrorState onRetry={() => attachmentsQuery.refetch()} /> : attachments.length === 0 ? <EmptyAttachments hasAny={(attachmentsQuery.data ?? []).length > 0} onUpload={openModal} /> : <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3">{attachments.map((item) => <AttachmentCard key={item.id} item={item} selected={selected.includes(item.id)} onToggle={() => toggle(item.id)} onDelete={() => remove(item)} />)}</div>}
     </section>
     {modalOpen && <AttachmentFormModal activePurchase={activePurchase} attachmentName={attachmentName} attachmentDate={attachmentDate} quotationName={quotationName} selectedFile={selectedFile} inputRef={inputRef} busy={create.isPending} message={message} onClose={resetModal} onSubmit={upload} onAttachmentNameChange={setAttachmentName} onAttachmentDateChange={setAttachmentDate} onQuotationNameChange={setQuotationName} onFileChange={setSelectedFile} />}
  </div>;
}

function AttachmentFormModal({ activePurchase, attachmentName, attachmentDate, quotationName, selectedFile, inputRef, busy, message, onClose, onSubmit, onAttachmentNameChange, onAttachmentDateChange, onQuotationNameChange, onFileChange }: { activePurchase: Purchase | null; attachmentName: string; attachmentDate: string; quotationName: string; selectedFile: globalThis.File | null; inputRef: RefObject<HTMLInputElement | null>; busy: boolean; message: string; onClose: () => void; onSubmit: (event: FormEvent) => void; onAttachmentNameChange: (value: string) => void; onAttachmentDateChange: (value: string) => void; onQuotationNameChange: (value: string) => void; onFileChange: (file: globalThis.File | null) => void }) {
  return <div className="fixed inset-0 z-40 flex items-end justify-center bg-[#020617]/70 p-0 backdrop-blur-sm sm:items-center sm:p-5"><div role="dialog" aria-modal="true" aria-labelledby="attachment-form-title" className="max-h-[92dvh] w-full max-w-[720px] overflow-y-auto rounded-t-3xl border border-[#334155] bg-[#0f172a] p-5 shadow-2xl sm:rounded-3xl sm:p-8"><div className="mb-7 flex items-start justify-between border-b border-[#334155] pb-5"><div><p className="mono text-[10px] uppercase tracking-[.18em] text-[#818cf8]">Novo registro</p><h2 id="attachment-form-title" className="mt-1 text-2xl font-extrabold tracking-[-.04em] text-[#f8fafc]">Enviar anexo</h2><p className="mt-2 text-sm text-[#94a3b8]">Informe os dados do arquivo antes de salvar.</p></div><button onClick={onClose} disabled={busy} aria-label="Fechar formulário de anexo" className="focus-ring grid h-9 w-9 place-items-center rounded-xl text-[#94a3b8] hover:bg-[#1e293b] disabled:opacity-50"><X size={18} /></button></div><form onSubmit={onSubmit} className="space-y-5"><div className="grid gap-5 sm:grid-cols-2"><Field label="Nome do anexo" required value={attachmentName} onChange={onAttachmentNameChange} placeholder="Ex.: Nota fiscal" testId="input-attachment-name" /><Field label="Data do anexo" required type="date" value={attachmentDate} onChange={onAttachmentDateChange} testId="input-attachment-date" />{activePurchase ? <div className="sm:col-span-2"><span className="mb-2 block text-xs font-extrabold text-[#cbd5e1]">Compra vinculada</span><div className="flex h-11 items-center rounded-xl border border-[#334155] bg-[#182337] px-3 text-sm text-[#cbd5e1]">{activePurchase.productName}</div></div> : <Field label="Nome da cotação" required value={quotationName} onChange={onQuotationNameChange} placeholder="Ex.: Cotação Loja X" testId="input-quotation-name" />}</div><label className="block"><span className="mb-2 block text-xs font-extrabold text-[#cbd5e1]">Arquivo <b className="text-[#818cf8]">*</b></span><input ref={inputRef} type="file" required={!selectedFile} className="sr-only" onChange={(event) => onFileChange(event.target.files?.[0] ?? null)} /><span className="flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-xl border border-dashed border-[#475569] bg-[#182337] px-4 py-3 text-sm text-[#cbd5e1] transition hover:border-[#818cf8]">{selectedFile ? <span className="min-w-0 truncate">{selectedFile.name}</span> : <span className="text-[#94a3b8]">Selecione um arquivo de até 8 MB</span>}<span className="flex shrink-0 items-center gap-2 rounded-lg bg-[#6366f1]/15 px-3 py-2 text-xs font-bold text-[#c7d2fe]"><Upload size={15} /> Escolher arquivo</span></span></label>{message && <p data-testid="status-attachment-modal" className={`text-xs ${message.includes("não") || message.includes("Não") || message.includes("Informe") || message.includes("Selecione") ? "text-[#fca5a5]" : "text-[#94a3b8]"}`}>{message}</p>}<div className="flex flex-col-reverse gap-3 border-t border-[#334155] pt-6 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} disabled={busy} className="focus-ring h-11 rounded-xl px-5 text-sm font-bold text-[#94a3b8] hover:bg-[#243249] disabled:opacity-50">Cancelar</button><button type="submit" disabled={busy} className="focus-ring flex h-11 items-center justify-center gap-2 rounded-xl bg-[#6366f1] px-6 text-sm font-bold text-[#f8fafc] shadow-[0_8px_18px_rgba(99,102,241,.2)] hover:bg-[#818cf8] disabled:opacity-60">{busy ? "Salvando…" : "Enviar anexo"} <Upload size={16} /></button></div></form></div></div>;
}
...
function AttachmentCard({ item, selected, onToggle, onDelete }: { item: Attachment; selected: boolean; onToggle: () => void; onDelete: () => void }) {
  ...
  const pdf = item.mimeType === "application/pdf";
   return <article data-testid={`card-attachment-${item.id}`} className={`group overflow-hidden rounded-xl border bg-[#182337] transition hover:border-[#6366f1]/70 ${selected ? "border-[#818cf8] ring-2 ring-[#6366f1]/20" : "border-[#334155]"}`}><div className="relative flex h-[170px] items-center justify-center overflow-hidden bg-[#0f172a]">{image ? <img src={item.downloadUrl} alt={item.attachmentName || item.fileName} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <div className={`grid h-16 w-16 place-items-center rounded-2xl ${pdf ? "bg-[#ef4444]/15 text-[#fca5a5]" : "bg-[#6366f1]/15 text-[#a5b4fc]"}`}>{pdf ? <FileText size={31} /> : <File size={31} />}</div>}<label className="absolute left-3 top-3 z-10 grid h-7 w-7 cursor-pointer place-items-center rounded-lg bg-[#0f172a]/75 backdrop-blur"><input data-testid={`checkbox-attachment-${item.id}`} type="checkbox" checked={selected} onChange={onToggle} className="h-4 w-4 accent-[#6366f1]" /><span className="sr-only">Selecionar {item.attachmentName || item.fileName}</span></label></div><div className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm font-extrabold text-[#f8fafc]" title={item.attachmentName || item.fileName}>{item.attachmentName || item.fileName}</h3><p className="mt-1 truncate text-xs text-[#94a3b8]">{item.purchaseId ? item.purchaseName : item.quotationName || "Cotação avulsa"}</p><p className="mt-1 truncate text-[11px] text-[#64748b]" title={item.fileName}>Arquivo: {item.fileName}</p></div><button data-testid={`button-delete-attachment-${item.id}`} onClick={onDelete} title="Excluir anexo" className="focus-ring grid h-8 w-8 shrink-0 place-items-center rounded-lg text-[#64748b] opacity-100 transition hover:bg-[#ef4444]/15 hover:text-[#f8fafc] sm:opacity-0 sm:group-hover:opacity-100"><Trash2 size={15} /></button></div><div className="mt-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-[#64748b]"><span>{formatBytes(item.fileSize)}</span><span>{formatDate(item.attachmentDate ?? item.createdAt)}</span></div><AttachmentDownloadButton item={item} /></div></article>;
}
...
function PurchaseEntry({ onDone, onAttachments, existing }: { onDone: () => void; onAttachments?: (purchaseId?: number) => void; existing?: Purchase }) {
  const queryClient = useQueryClient();
  ...
  const submit = (event: FormEvent) => {
    ...
    const shouldAttach = submitter instanceof HTMLButtonElement && submitter.dataset.action === "save-attachments";
    const totalValue = parseMoney(form.totalValue);
    if (!form.purchaseDate || !form.supplier || !form.productName || !form.recipient || !form.base || !Number.isFinite(totalValue) || totalValue < 0 || Number(form.quantity) < 1) { setError("Preencha os campos obrigatórios para salvar a compra."); return; }
    const data: PurchaseInput = { purchaseDate: form.purchaseDate, deliveryDate: form.deliveryDate || null, supplier: form.supplier.trim(), productName: form.productName.trim(), recipient: form.recipient.trim(), base: form.base.trim(), quantity: Number(form.quantity), totalValue, paymentMethod: form.paymentMethod, source: PurchaseInputSource.manual, ...(!existing ? { status: form.status } : {}) };
    const onSuccess = (saved?: Purchase) => { queryClient.invalidateQueries({ queryKey: getListPurchasesQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetDashboardSummaryQueryKey() }); if (shouldAttach && onAttachments) onAttachments(saved?.id ?? existing?.id); else onDone(); };
